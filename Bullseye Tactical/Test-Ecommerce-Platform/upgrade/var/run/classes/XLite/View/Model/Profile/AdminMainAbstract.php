 ...
 */
abstract class AdminMainAbstract extends \XLite\View\Model\AModel
{
    /**
     * Form sections
     */
    ...
    protected function getFormFieldsForSectionMain()
    {
        ...
        if ($profile) {
            ...
            } elseif ($profile->getOrdersCount()) {
                if (isset($this->mainSchema['login'])) {
                    $this->mainSchema['login'][static::SCHEMA_COMMENT]
                        = static::t('E-mail will also be updated in all the related orders.');
                }
            }
            ...
            if ($profile->getProfileId()
                ...
                $this->mainSchema['password'][self::SCHEMA_COMMENT] = static::t(
                    'Send reset link',
                    ['url' => $this->buildURL('profile', 'recover_password', ['profile_id' => $profile->getProfileId()])]
                );

                unset($this->mainSchema['password_conf']);

                $mainSchema = [];
                foreach ($this->mainSchema as $name => $schema) {
                    $mainSchema[$name] = $schema;
                    ...
                    if ($name === 'password') {
                        ...
                        if (null === $this->currentAction) {
                            $mainSchema['password_current'][self::SCHEMA_REQUIRED] = true;
                        }
                    }
                }
     ...
     * @return array
     */
    protected function validateFormFieldPasswordCurrentValue($field, $formFields)
    {
        $result = [true, null];
        ...
        if ($data
            ...
            if (($data['login'] !== $savedData['login'] || !empty($data['password']))
                && !Auth::comparePassword($savedData['password'], $data['password_current'])) {
                $result = [false, 'Incorrect password. Please try again.'];
            }
        }
